...
open_positions, before Carry-forward. It sorts each fill against two lists — staged/known-member
Aegis tickers (silently confirmed) and a persistent non-Aegis exclusion list (silently dropped,
never re-flagged) — and AUTO-INCLUDES anything matching neither as a confirmed Aegis position
(D-106, PM ruling 2026-08-25, reconfirmed 2026-09-06): counted in every metric immediately,
flagged `auto_included_unrecognized` at HIGH severity so it can't be missed on screen, and
persisted onto the membership file (`--membership`) so it confirms silently from the next run on.
The PM's only lever is `reject` (mark_rejected) afterward if it turns out not to be Aegis's —
there is deliberately no separate confirm-first step for the common case. (`mark_confirmed` still
exists for the rarer case of a position hand-set to some other non-confirmed status.) **This
behaviour was found reverted to an older `pending_review`-parking design during the v5.3 rebuild
— if you are reading this while `classify_aegis_status` sets `pending_review` instead of
`confirmed` on an unmatched fill, that is a regression, not a redesign; the PM has not changed
this ruling.**
...
Usage:
  python3 tools/held_book_refresh.py classify --journal today.json --exclusions data/persistent/non_aegis_exclusions.json [--staged staged.json] [--membership data/persistent/aegis_membership.json] [--out today.json]
  python3 tools/held_book_refresh.py confirm --journal today.json --ticker TICK [--out today.json]
  python3 tools/held_book_refresh.py reject --journal today.json --ticker TICK --exclusions data/persistent/non_aegis_exclusions.json [--membership data/persistent/aegis_membership.json] [--reason "..."] [--out today.json]
...
def classify_aegis_status(journal, staged_tickers, exclusions):
    """Run once, right after broker fills build open_positions (Operation 1), before anything
      ...
      3. Neither -> AUTO-INCLUDED as aegis_status = confirmed (D-106, PM ruling 2026-08-25,
         reconfirmed 2026-09-06 after this behaviour was found reverted in the v5.3 rebuild).
         Real capital, real risk, counted in every metric from day one — the PM does not have to
         act for it to be managed correctly. It is flagged `auto_included_unrecognized`, HIGH
         severity, so it is impossible to miss on screen. The PM's only lever is `reject`
         afterward (mark_rejected) if it turns out not to be Aegis's; there is no separate
         "confirm" step to take for the common case. `classify`'s caller is expected to persist
         the ticker onto the membership file the next run's `--staged` reads (see
         `cmd_classify`'s `--membership-out`), so it is silently `confirmed` from here on and
         never re-flagged. Do NOT reintroduce a `pending_review` parking state here without a
         PM ruling superseding this one — that was the exact behaviour this ruling replaced.
    staged_tickers: iterable of tickers this run's fills-reconciliation expected (from the
    gatekeeper's staged orders, OR the persistent membership file — see run_post_market.sh).
    exclusions: dict from load_exclusions(). Returns a report."""
    staged = set(staged_tickers or [])
    confirmed, excluded_non_aegis, auto_included, remaining = [], [], [], []
    for pos in journal.get("open_positions", []) or []:
        ...
        if t in staged or pos.get("aegis_status") == "confirmed":
            ...
            _unflag(journal, t, "pending_review")
            _unflag(journal, t, "auto_included_unrecognized")   # clears a flag from before a later exclusion
        else:
            pos["aegis_status"] = "confirmed"
            auto_included.append(t)
            remaining.append(pos)
            _flag(journal, t, "auto_included_unrecognized",
                  "fill not matched to a staged Aegis order or the known-membership file — "
                  "AUTO-INCLUDED as a confirmed Aegis position (D-106) and counted in every "
                  "metric. PM's only lever: reject it to remove and permanently exclude.",
                  severity="high")
    journal["open_positions"] = remaining
    return {"confirmed": confirmed, "excluded_non_aegis": excluded_non_aegis,
            "auto_included_unrecognized": auto_included}
...
def mark_rejected(journal, ticker, exclusions_path, reason="PM-rejected, non-Aegis", today=None,
    ...
    """PM override on a D-106 auto-included (or legacy pending_review) position. Removes it from
    open_positions (it was never an Aegis holding) and records it once on the persistent
    exclusion list, so it never resurfaces again — the exclusion list IS the record, there is no
    separate archive. If `membership_path` is given (the same file `classify` was told to persist
    auto-inclusions onto), the ticker is also stripped from it — otherwise a rejected name that
    was already written there by a prior run's auto-include would keep matching `staged` on
    tomorrow's classify and silently override this rejection. Caller must re-run
    portfolio_metrics.py compute afterward, same as mark_confirmed."""
    journal["open_positions"] = [p for p in journal.get("open_positions", []) or []
                                 if p.get("ticker") != ticker]
    _unflag(journal, ticker, "pending_review")
    _unflag(journal, ticker, "auto_included_unrecognized")
    add_exclusion(exclusions_path, ticker, reason, today or journal.get("date"))
    if membership_path:
        try:
            members = _load(membership_path)
        except FileNotFoundError:
            members = []
        if ticker in members:
            members = [m for m in members if m != ticker]
...
def cmd_classify(args):
    ...
    membership_out = args.membership_out or args.membership
    if membership_out and report["auto_included_unrecognized"]:
        # D-106: persist today's auto-includes onto the membership file so tomorrow's classify
        # matches them under `staged` and confirms silently, with no re-flag every day.
        try:
            members = _load(membership_out)
        except FileNotFoundError:
            members = []
        added = [t for t in report["auto_included_unrecognized"] if t not in members]
        if added:
            members = sorted(set(members) | set(added))
            with open(membership_out, "w") as fh:
                json.dump(members, fh, indent=1)
            report["membership_file_updated"] = membership_out
            report["membership_added"] = added
...
def cmd_reject(args):
    ...
    print(json.dumps({"op": "reject", "out": out, "ticker": args.ticker,
                      "exclusions": args.exclusions, "membership": args.membership,
                      "note": "re-run portfolio_metrics.py compute to reflect this"}, indent=1))
...
def selftest_aegis_status(_args=None):
    """D-106 (PM ruling 2026-08-25, reconfirmed 2026-09-06): an unrecognized fill is
    AUTO-INCLUDED as confirmed, not parked in pending_review. This selftest replaces the
    pre-D-106 version that asserted the old parking behaviour."""
    import tempfile
    tmpdir = tempfile.mkdtemp()
    excl_path = os.path.join(tmpdir, "non_aegis_exclusions.json")
    membership_path = os.path.join(tmpdir, "aegis_membership.json")

    journal = {"date": "2026-07-28", "open_positions": [
        {"ticker": "AAA", "qty": 10, "entry": 50.0},                 # staged -> confirmed
        {"ticker": "MARA", "qty": 5, "entry": 20.0},                 # unmatched, unexcluded -> auto-included
    ...
    assert r1["auto_included_unrecognized"] == ["MARA"], r1
    ...
    assert journal["open_positions"][1]["aegis_status"] == "confirmed", \
        "D-106: an unrecognized fill is confirmed immediately, never parked as pending_review"
    kinds = {f["type"] for f in journal.get("review_flags", [])}
    assert "auto_included_unrecognized" in kinds, journal.get("review_flags")
    mara_flag = [f for f in journal["review_flags"] if f["ticker"] == "MARA"][0]
    assert mara_flag["severity"] == "high", mara_flag

    # PM's ONLY lever is reject — there is no separate confirm step for the common case, but
    # mark_confirmed still exists and still works (e.g. for any position hand-set to
    # pending_review some other way) and clears whichever flag is present.
    ...
    # PM overrides a second auto-included name instead of leaving it — reject removes it AND, if
    # a prior day's classify already persisted it onto the membership file, strips it there too
    # so it can never silently re-match `staged` tomorrow.
    with open(membership_path, "w") as fh:
        json.dump(["AAA", "DDD"], fh)   # DDD already auto-included & persisted on a prior day
    journal["open_positions"].append({"ticker": "DDD", "qty": 2, "entry": 3.0,
                                      "aegis_status": "confirmed"})
    _flag(journal, "DDD", "auto_included_unrecognized", "fill not matched to a staged order")
    ...
    assert "DDD" not in _load(membership_path), \
        "reject must strip a rejected ticker back out of the membership file"
    assert "AAA" in _load(membership_path), "reject must not touch unrelated membership entries"

    # DDD must never resurface as auto-included again — the exclusion list catches it upstream
    ...
    assert "DDD" not in r2["auto_included_unrecognized"], \
        "a previously-rejected ticker must be excluded silently, never re-flagged or re-included"

    print("held_book_refresh selftest_aegis_status OK — staged fills confirm silently, "
          "previously-rejected fills are dropped silently and never re-included, genuinely new "
          "fills are AUTO-INCLUDED as confirmed and flagged once high-severity (D-106); "
          "confirm/reject correctly mutate open_positions and review_flags, and reject persists "
          "to the exclusion list and strips the membership file.")
...
def main(argv=None):
    ...
    cl.add_argument("--staged", help="JSON file: list of staged/known-membership Aegis tickers")
    cl.add_argument("--membership", help="D-106: JSON file (list of tickers) to persist today's "
                     "auto-included unrecognized fills onto, so tomorrow's classify confirms "
                     "them silently. Same shape as --staged; often the same file.")
    cl.add_argument("--membership-out", help="Alias of --membership, wins if both given")
    cl.add_argument("--out")
    ...
    rj.add_argument("--membership", help="D-106: also strip the ticker from this membership "
                     "file if a prior auto-include already put it there")
