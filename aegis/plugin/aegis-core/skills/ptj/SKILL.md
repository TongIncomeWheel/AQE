...
**v5.3 (2026-09-05) — exit 1 caused ONLY by the two git pushes is not a degraded close. D-110 (PM ruling 2026-09-06) makes the fix-up MANDATORY, not optional prose.** On Cowork the sandbox git proxy returns 403 for this repo, so jobs 8 and 12 report `pushed: false` every day — expected, not a fault. **This is exactly the failure mode that shipped a real incident: a 2026-09-04 run pushed successfully via the connector minutes after the script finished, but the gate was never re-stamped, and the fund ran for two days believing that day's book was "degraded" when it was actually fine and fully on GitHub.** The gate does not fix itself — a session that walks away after seeing "committed locally, not pushed" without doing the three steps below leaves the SAME stale-partial trap for the next reader. Whenever EITHER push job shows `"committed locally, not pushed"` and NO OTHER job failed, this is a MANDATORY 3-step checklist — not complete, and not relayable as fixed, until all three steps below have actually run, in order:

  **STEP 1 — PUSH.** Push via the connector. Delegate to a subagent (or do it directly) using the GitHub MCP connector's `push_files`/`get_file_contents` against `TongIncomeWheel/AQE` main — the exact files `git_sync.py` had already committed locally.

  **STEP 2 — VERIFY.** `get_file_contents` the pushed paths back from `main` and confirm they match what was pushed (JSON: semantic compare via `json.loads()`, tolerant of `\uXXXX` vs literal unicode; plain text: byte-for-byte). Do not proceed to step 3 on an assumption that the push worked — read it back from GitHub and check it.

  **STEP 3 — RE-STAMP THE GATE.** This is the step that was skipped in the 2026-09-04 incident, and skipping it is what turns a successful push into a two-day-long false "partial." Run `python3 tools/phase_gate.py stamp --phase post_market --status ok --journal-date <DATE> --note "connector push confirmed landed, commit <SHA>"` (add `--date <the run's calendar date>` if re-stamping after the fact, as opposed to during the same run). **A push that reached GitHub but left the stamp at `partial` is functionally the same as a push that never happened** — `/pma`'s and `/premarket`'s gates read the STAMP, not GitHub, and will sit blocked or degraded indefinitely until this runs.

Only after step 3 does this relay as "Book closed and pushed (connector)". Page only if a job OTHER than the two pushes failed — but never call the run finished on a push failure without having attempted all three steps above, and never call it finished after step 1 or step 2 alone.
