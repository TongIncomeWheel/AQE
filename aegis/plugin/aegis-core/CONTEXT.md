...
**Session bootstrap — FIRST action, any session (D-49):** run `tools/preflight.py`. The ONLY secret Aegis needs locally is `GITHUB_PAT` — the autonomous git push credential (there is NO GitHub connector, D-48); market data and the book ride OAuth connectors (FMP/Tiger/Drive — IBKR retired D-98, no residual capital), no local secret. The token lives ONLY in gitignored `config/.env` and is **NEVER shipped in the install pack** (a secret in a distributable is a leak — the pack carries `config/env.example`, the template, not the key). If preflight reports `GITHUB_PAT` MISSING, **ASK the PM** to provide it — `Save GITHUB_PAT=<fine-grained PAT, Contents:Read+Write on the repo> into aegis/config/.env` — save it, then continue. That is how a fresh workspace self-heals its own config. Because the Cowork workspace persists, the token is set ONCE and stays; preflight is the safety net for a new deployment or a lost `.env`.

**How you interact — grounded in Claude Cowork (D-31, delivery corrected D-76):** Aegis lives in ONE Cowork **workspace** — a persistent file store that syncs across your devices, not a chat. The workspace IS the memory. Three things touch it: (1) **scheduled tasks** run remotely with your app closed — premarket, market-hours watch, post-market, weekly — each its own session reading/writing the workspace files; these do the durable COMPUTE and push to git, but their sessions are viewed independently and are **not surfaced on mobile yet**, so they are NOT how you SEE results; (2) **your cockpit** is the Cowork chat you actually have open — run **`/cockpit arm`** in it and each loop's output is delivered HERE (via `send_later`, a few minutes after the cron pushes) instead of dying in an invisible scheduled session; (3) commands (`charter/commands.md`) — you type `/status`, `/ops`, `/plan` and the session reads today's files and answers. A FRESH chat reconstitutes fully: it loads this file + charter, then reads today's shelf from git — so cards are correct even in a brand-new window (they read files, never chat memory). **On any fresh Aegis session, FIRST surface whether a cockpit is armed (read `data/cockpit/armed.json`); if not, tell the PM "cockpit not armed — run `/cockpit arm` to get live delivery here" (D-76). A send_later cadence dies when its session is truly closed, so re-arming in a new chat is the one manual step — compute+git are durable, nothing is lost in the gap.** Precise SGT timing uses the scheduled-task tool (cron). Kimi runs the same pattern in parallel, read-only until you confirm migration; only one deployment is ever armed for live orders.
...
| Journal substance | Multibroker reconcile as execution truth · dynCap roll · SL audit (live vs reference, MATCH/MISMATCH/MISSING) | ptj skill (CLOSE mode) + journal contract |
...
```
   ...
   (plugin + subagents)                           (Agent Skills + swarm + mcp.json)
        ...
        └──────┬───────────┬──────────────────────────┬─────────────┘
             Tiger MCP   Alpaca MCP                   FMP
             (REUSED,    (REUSED,                     (bars, screener,
             cloud: sole  cloud: Greeks                 quotes)
             broker —     15-min delay)
             spot, positions,
             staging p/c)
   (IBKR MCP retired D-98 — no residual capital, account fully wound down)
        ┌──────────────────────────────────────────────┐
...
3. **Stop defined** (D-38) — the AQE bracket carries a DEFINABLE stop (a structural stop, else the ATR-fallback stop), copied verbatim, so dollar-risk is bounded. That is the ONLY hard bar here. Bracket QUALITY (ATR-distance, R:R, risk%) is a SOFT flag surfaced at deliberation, NOT a refusal — a high-beta breakout with a wide stop that misses R:R 2.0 still passes; it is sized DOWN, not cut. Risk is controlled by size, never by refusing the setup.
...
**16:00** — plan carries CLOV: committee ADVANCE 6/9 (dissent: Steenbarger, crowding note recorded), trigger "break of $4.20", AQE bracket: stop 3.86 (fib_618, ATR-distance 1.4), TP1 4.55, TP2 4.95, R:R to TP2 = 2.03, risk% = (4.22−3.86)/4.22 = **8.5%** — a wide stop on a high-beta name. Pre-D-38 this was auto-refused at check 3; **now (D-38) it is NOT cut** — the stop is defined, so dollar-risk is bounded. The wide stop rides as a SOFT quality flag on the plan line ("wide stop, risk 8.5%, R:R 2.03 — high-beta momentum call"); the committee/PM see it and decide, and Risk **sizes it down** so the R budget still holds. That is the frustration D-38 removed: good breakouts are no longer filtered out by a bracket-validity gate.

- Check 4, sizing on the ACTUAL (wide) stop: 1R = $1,088.62; risk/share = 4.22 − 3.86 = $0.36 → shares_R = floor(1088.62 / 0.36) = **3,023** (a wider stop simply buys fewer shares; the dollar risk still lands at ~1R). Vol-cap: vol_30d 0.55 → daily vol/share = 4.22 × 0.55/√252 = $0.146; cap$ = 1.5% × 72,574 = $1,088.6 → shares_vol = 7,456. Final = min = **3,023 shares** (~$12,757 position, capped by R on the wide stop).
- Check 5: post-add β30d 0.29 ✓, combined stop risk 3.1% + 1.5% = 4.6% < 5% ✓, leverage ✓, VaR ✓.
- Checks 1,2,6,7 ✓ → **preview emitted**: `BUY 3023 CLOV LMT 4.22 · stop plan 3.86 (post-fill) · TP1 4.55 / TP2 4.95 · 1.0R = $1,088 · wide-stop soft flag (risk 8.5%, R:R 2.03) · checks 7×PASS`.

**21:00** — you stage it yourself in the broker as a conditional order. **02:47** — CLOV breaks 4.20; your staged order fills at **4.23**. Gatekeeper post-fill: actual risk = 3023 × (4.23−3.86) = **$1,119** vs budget $1,088 → delta **+$31 < $50 tolerance**, no flag (at +$50+ it would instruct a quantity trim). Emits the metrics line (exposure, leverage, β, combined stop 4.7%, sector weights) and the stop instruction: **"Stage stop: SELL 3023 CLOV STP 3.86 — at actual fill basis."** You staged the stop with the bracket at 21:00, so this is a verification, not a wake-up. **10:00** — the fire, the fill, the delta and the metrics are the first lines of your morning summary.
...
2. Fill `config/endpoints.json`: Tiger MCP URL, Alpaca MCP URL (reused cloud services), FMP key env. (IBKR gateway choice retired D-98 — no longer part of setup.)
