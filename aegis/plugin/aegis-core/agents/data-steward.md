...
1. **Fetch from `TongIncomeWheel/AQE` branch `main`** (always `ref: refs/heads/main`):
   ...
   - `aegis/output/voice_packets/` — the pre-sliced Round-1 packets + `packet_stamp.json` (v5 ruling #4). I re-hash every file against the stamp AND hash the RESOLVED per-voice menus (2026-09-15: each voice's menu now lives inside its own `canon/<voice>/canon.lock.yaml` as a `menu:` block — the standalone `voice_menus.json` this used to be is retired) against the stamp's `menus_sha256`. Any mismatch or absence ⇒ degradation `packet_stamp_mismatch` — I still deliver the canonical export so PREPARE can slice locally (the retained fallback). I never "fix" a packet.
   - Tools, fresh, never cached: `aegis/skills/premarket-analysis/tools/pma_pipeline.py`, `pm_lens.py`, `registrar.py`, `purity_check.py`; `canon/*/canon.lock.yaml` (each voice's methodology AND its `menu:` field list, one file), `contracts/vote_round2.schema.json`; `stages/S6B_post_committee.md`.
