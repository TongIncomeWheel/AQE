...
8. **Trade journal** (D-115, from `data/eod/DATE/trade_journal_DATE.md` — print the file's tables, do not recompute): the Aegis stats line, Aegis only — closed trades **since the last run** in full (out, in, ticker, qty, entry, exit, gross, fees, net, %, days, R), the open positions table, and every line under "Reconciliation" and "Flags" verbatim — those are the PM's call. The full closed-trade history stays in the file and on GitHub; on screen, print the last ten unless the PM asks for more. Non-Aegis names and option legs never appear in this record.
...
|---|---|---|---|
| **0** | `ok` | every job passed — **including** a run whose only push "finding" was the known, every-day sandbox git-proxy block (D-111, see below); that is a normal 0, not a degradation | Relay the checklist. `run_ok`. |
| **1** | `partial` | the book of record is sound but something REAL is degraded — a push that failed for a reason OTHER than the known sandbox block, a later job that failed | Relay the checklist naming the failing lines. **PAGE.** |
...
**D-111 (PM ruling 2026-09-06 — "why even show the fail which is useless, inaccurate... remove it to avoid false alarms").** On Cowork the sandbox git proxy returns 403 for this repo on jobs 8 and 12, every single run — not sometimes, every time. The script used to label that `FAIL` and knock the whole run down to `partial`, which manufactured an alarm-looking result out of something that was never actionable and always self-healed. It no longer does: `run_post_market.sh` recognizes this EXACT signature in `git_sync.py`'s own output and records it as `expected`, not `FAIL`, and does not degrade the run for it. **A push failure that does NOT match this known signature — a real auth problem, a genuinely different error — still records `FAIL` and still degrades the run to `partial`; this removes a false alarm, it does not blind the run to a real one.**

**This does NOT remove the connector delivery requirement — D-110 (PM ruling 2026-09-06) still runs every time, unconditionally.** The exact incident that created D-110 (a 2026-09-04 run whose connector push landed fine but whose gate was never re-stamped, so the fund believed a clean book was "degraded" for two days) is why the two rulings are not the same fix: D-111 stops the SCRIPT from crying wolf about the routine, expected half of the push; D-110 makes sure the REAL other half — actually delivering the book to GitHub via the connector — never gets skipped just because the script now looks calm. Whenever a job shows `expected` for either push, this sequence still runs, and step 3 still matters even though exit code 0 no longer forces the question:
  1. **Push via the connector.** Delegate to a subagent (or do it directly) using the GitHub MCP connector's `push_files`/`get_file_contents` against `TongIncomeWheel/AQE` main — the exact files `git_sync.py` had already committed locally.
  2. **Verify it landed.** `get_file_contents` the pushed paths back from `main` and confirm they match (JSON: semantic compare via `json.loads()`, tolerant of `\uXXXX` vs literal unicode; plain text: byte-for-byte).
  3. **If verify FAILS — downgrade the gate yourself.** Run `python3 tools/phase_gate.py stamp --phase post_market --status partial --journal-date <DATE> --note "connector push did not verify — see <detail>"`. This is now the RARE, real-problem direction (the gate already reads `ok` from the script) rather than the routine upward correction D-110 used to require every single day.
Only after connector delivery is confirmed does this relay as "Book closed and pushed (connector)". Page if a job other than the two pushes failed, OR if step 2's verify ever fails — never assume a calm exit code means GitHub actually has the files without having checked.
...
**Aegis equity membership (job 3) — D-106, PM ruling 2026-08-25, reconfirmed 2026-09-06.** The PM runs other books on the same brokers, so not every fill is Aegis's. A fill matching a staged order or the known-membership file is `confirmed` silently; a fill matching the persistent exclusion list (a ticker the PM has already rejected) is dropped from the journal entirely and never re-flagged; **anything matching neither is AUTO-INCLUDED as `confirmed`** — real capital, counted in every metric immediately — and flagged `auto_included_unrecognized`, **high severity**, once for the PM. The PM's only lever is to `reject` it afterward if it turns out not to be Aegis's; there is no separate confirm step to take first. `held_book_refresh.py classify` persists each auto-include onto `data/persistent/aegis_membership.json` (`--membership`) so it confirms silently from the next run on, never re-flagged. See `print-trade-journal` for the full logic. **With no staged-orders list and no membership file on disk, every unmatched fill still auto-includes. That is the designed behaviour, not a fault — and if you ever see an unmatched fill sitting as `pending_review` instead, that is this ruling having regressed, not a redesign (it happened once already, during the v5.3 rebuild).**
