...
- `dynCap = allocated_capital + realised P&L (closed) + UNREALISED P&L (open, marked) = current Aegis equity` (RB:capital.dyncap_method — D-41 mark-to-market, revises D-21) — maintained in `data/persistent/dyncap_ledger.json` by `tools/dyncap_ledger.py` (Operations refreshes it from the fresh Aegis PTJ each premarket; I read it via `get_dyncap()`). It MOVES with unrealised P&L — so I size against what is actually held, and it de-sizes in drawdown (procyclical, the PM's choice). A mark older than today's PTJ is stale — I do not size on it.
