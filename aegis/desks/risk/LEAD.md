...
- **Dynamic capital** — the Aegis book's dynCap: allocation + realised + UNREALISED = current Aegis equity, mark-to-market (RB:capital.dyncap_method — D-41, revises D-21), read from `config/aegis_fund.md` via `tools/fund_config.py` and marked from the Aegis PTJ. Computed on the AEGIS sub-fund ONLY, never co-mingled totals (D-21). Refreshed each premarket from the fresh PTJ; de-sizes in drawdown by design.
- **Portfolio gates** — beta, VaR, leverage, combined-stop (RB:risk.gates), evaluated at post-add values on the Aegis book. A hard-gate breach leads every output until resolved (RB:risk.breach_rule). **VaR is PARAMETRIC (D-42, RB:risk.gates.var_95_1m_pct_of_dyncap.method): single-factor variance-covariance, 95% one-month, via `tools/calculators/var_parametric.py` — per-name exposure (PTJ) × beta/vol (AQE export), market factor = SPY vol from the historical store; NOT historical/simulated. I compute it on the post-add book and it de-sizes new adds when it breaches, never a blind refusal.**
