...
- **Secrets** live in a gitignored file — `aegis/config/.env` (copy from `aegis/config/env.example`). Required: `FMP_API_KEY` `[PM]`. **`GITHUB_PAT` `[PM]` is REQUIRED for the autonomous commit/push (D-48)** — a fine-grained PAT with **Contents: Read+Write** on `TongIncomeWheel/AQE`. There is NO GitHub MCP connector, so this token is how unattended scheduled sessions push results (`tools/git_sync.py` reads it from env and pushes with no human step). It lives ONLY in gitignored `aegis/config/.env` — never a tracked file. Confirm the deployed workspace persists `config/.env` across scheduled-task firings (the token must be readable by each fresh session); if the platform provides a secret/env store for scheduled tasks, set it there instead. **The token is NEVER bundled in the plugin/install pack** — `config/env.example` (the blank template) ships, the key never does. `tools/preflight.py` (run first in every session per the CONTEXT.md bootstrap, D-49) detects a missing `GITHUB_PAT` on a fresh/empty workspace and prompts the PM to paste it once — so config self-heals instead of silently failing to push. Alpaca read-only keys are already embedded per PM ruling.
- **Notifications (D-45/D-46) — PILOT: nothing to configure.** On Claude Cowork the channel is native (`RB:notifications.channel: cowork`): each run's message rides the scheduled-task **push** to the PM's phone; `notify.py` emits it as session output. The WhatsApp channel + external watchdog secrets (`WHATSAPP_*` / `TWILIO_*` / `WATCHDOG_URL` in `aegis/config/.env`) are the **POST-MIGRATION** path (local box / Kimi) and are left blank for the pilot — do NOT wire them now. They only activate when `AEGIS_NOTIFY_CHANNEL=whatsapp`.
...
- **CHECK:** a trivial skill loads and both `/fa` (book from state) and `/ops` (system liveness, D-45) render without error, using the NEWLY installed plugin (verify skill/version, not just that *a* plugin responds — a stale prior plugin can mask this check). `/ops` shows the alert channel as **cowork-native** (the pilot default, D-46) — the only expected PARTIAL on a fresh box is first-run/git-head state, not a channel problem.
...
- Create the scheduled tasks on the app's own scheduler (NOT the in-process cron). The five tasks, their times, and the prompt template live in `aegis/SCHEDULERS.md`. Use the `create_trigger` scheduled-task tool; each fires a fresh session that pulls the repo, runs its phase, and commits.
- **Liveness on the pilot (D-46)** `[PM]`: the primary liveness signal is native — turn on each loop task's `notifications: {push:true}` so its run_ok/run_fail message reaches your phone as a Cowork push. Optionally add a light **pre-run heartbeat** as its own `create_trigger` task firing `tools/notify.py pre_run` ~`RB:notifications.pre_run_minutes` before each key run (a "starting soon, alive" push). **On Cowork the external watchdog and the Windows `schtasks`/batch path in `aegis/NOTIFICATIONS_SETUP.md` are NOT used — that entire guide is the POST-MIGRATION (local box) path.** A dead-man's-switch only matters once WE run the infra; on Cowork a missed scheduled task is surfaced by the platform itself. **CHECK:** `list_triggers` shows the loop tasks (and any optional heartbeat) with push notifications enabled.
...
- **Self-heal layer UAT (D-45/D-46, Cowork-native):** `/ops` renders the liveness card and shows the channel as cowork-native; `notify.py run_fail --dry-run` renders and its failure page is visibly distinct from a green ping; a `notify.py run_ok`/`pre_run` on Cowork emits the message that rides the scheduled-task push (confirm a test scheduled task's push reaches the phone); `self_heal.py <loop> --failure ptj_pull --dry-run` reports **healed** (transient) while `--failure tripwire` reports **stand_down / not healed** (gate — never auto-healed); confirm NONE of `/heal /recover /repull /reseed` can place an order — a recovered loop reaching execution still stops at a gatekeeper preview. *(The external-watchdog / dead-man's-switch check is POST-MIGRATION only — skip it on the Cowork pilot.)*
