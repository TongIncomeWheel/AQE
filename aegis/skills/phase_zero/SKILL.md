...
2. **Branch on the exit code. This is the whole skill.**
   - **0 = READY** → run `python3 tools/phase_gate.py claim --phase premarket_build`. If the claim is **won**, fire the premarket build — `mcp__claude-code-remote__fire_trigger` on **RB:schedule.phase_zero.fires_trigger_id**, the EXISTING "Aegis premarket build" scheduled task. Fire that one, never a new session started by hand: there is one build path and this skill only decides *when* it runs. Then stop. If the claim is **lost**, today's build already fired on an earlier firing of this task — **do nothing, say nothing, exit**. The claim is what makes a repeating schedule safe; never fire without winning it.
...
- **The premarket build's own cron must be OFF while Phase 0 owns the firing.** Leaving both live gives the morning two ways to start the same build — Phase 0 on READY and the old fixed 10:00 cron regardless — and the 10:00 one would run on whatever state exists, which is the exact thing this gate was built to stop. One firing authority, and it is this skill. (The claim latch stops Phase 0 double-firing itself; it does not know about a cron firing the trigger behind its back.)
