---
name: pma
description: "PMA — Premarket Analysis, the committee morning. Trigger: /pma. One command, repeatable by hand or on a scheduler: pulls the day's AQE export + Crown macro file from the repo, spawns the macro voices and the 9-seat blind swarm, tallies, ranks to the Phase-4 cap, runs challenge + fundamentals + Round-2 deliberation on the capped set, closes consensus arithmetically, auto-brackets everything phase-3+, locks every verdict (and every cap-dropped NEAR-MISS) into the accountability ledger, mechanically generates the REPEAT WATCH table from that ledger (never hand-typed), renders the fixed 8-section CIO report with ticker cards, passes the S7Q quality gate, publishes to git + project doc + on screen. Analysis only — no brokers, no orders, no sizing (constitution law 1 untouched). v4.3, PM-ratified 2026-08-18 (adds mandatory verdict-lock + automated repeat-watch, closing the 'automated not by hand' standing rule); validated end-to-end on the 2026-08-17 dry run (25 spawns, 0 errors) and the 2026-08-18 live run."
---

# /pma — PREMARKET ANALYSIS (v4.3 operational card — supersedes the v0.4 design card)
...
State: `data/pma/<date>/` (run artifacts) · `data/pma/phase4_ledger.json` (rolling repeat ledger) · `data/pma/verdict_ledger.json` (accountability ledger — every verdict AND every cap-dropped NEAR-MISS, locked live, WRITE-ONCE per date+ticker) · `aegis/reports/pma/<date>.md` + `latest.md`.
