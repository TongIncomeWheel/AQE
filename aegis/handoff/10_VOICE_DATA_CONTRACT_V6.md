...
**SUPERSEDED 2026-09-15 — do not act on this document.** This whole effort was a third file
(`voice_menus.v6.json`, generated by `bind.py`/`render.py` from canon + the standalone
`voice_menus.json`) meant to stay in sync with the other two. It never shipped — blocked for
months on a glossary file that never made it into the repo — and the problem it was trying to
solve (two files describing one voice, drifting apart) is now gone a different way: PM instruction
2026-09-15 merged each voice's menu directly INTO its own `canon/<voice>/canon.lock.yaml` as a
`menu:` block, and deleted BOTH the standalone `voice_menus.json` files this document's tooling
reads from. There is exactly one file per voice now, read directly by both AQE's own code and the
PMA pipeline via `pma_pipeline.py`'s `load_menus_from_canon()` — no generation step of any kind,
scheduled or otherwise. `bind.py`/`render.py` in this folder will not run against the current repo
(their input file is gone). Kept here as historical record only; see
`src/pipeline/voice_packets.py`'s 2026-09-15 comment for the full account.
