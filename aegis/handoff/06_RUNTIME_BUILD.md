...
| **Orchestrator** | **LangGraph** *or* a lean asyncio orchestrator | LangGraph gives stateful loops, **checkpoint/resume** (SH-R1) and **human-in-the-loop interrupt** (approval gates, H1) out of the box, model-agnostic. If you want zero framework lock-in, a custom asyncio orchestrator that reads the skill cards + dispatches is very viable (the kernel is mostly deterministic + model calls). |
...
| **Tools/connectors** | run as **local MCP servers** (parity with today) or direct Python clients | FMP REST · Tiger `tigeropen` SDK · IBKR via **IB Gateway + `ib_insync`** (or IBKR Web API) · Google Drive API (service account) |
