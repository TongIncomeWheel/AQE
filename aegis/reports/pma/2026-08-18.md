...
*Simplified per PM request 2026-08-18 ("I only need the ticker, date appeared, % vs last COB, state"), then automated per PM request the same day ("I want it automated, not by hand, which means gaps and forget will occur"). This table is machine-generated by `pma_pipeline.py repeat-watch` — cross-references `phase4_ledger.json` (who repeated, and when) against `verdict_ledger.json` (what price/verdict was locked each of those dates) and prints the table verbatim, no hand transcription. 2026-08-17 was seeded into the verdict ledger as a one-time historical backfill (marked `record_source: backfilled_manual_2026-08-18` in the data, distinct from every row from today forward, which the pipeline locks live via `record-verdicts`) — from today on, every appearance a repeat ticker makes gets locked automatically the day it happens, so there is nothing left to reconstruct by hand. If a future appearance has no matching verdict-ledger row, the tool prints an explicit gap rather than guessing or silently dropping the row. Window is currently 2 sessions deep and will build toward a true 5-session view as more sessions run.*
...
- **Phase-4 REPEAT flags — now a standing, visible section (§4), not buried here, and fully automated.** Moved out of disclosures per PM request 2026-08-18 ("I would have wanted to see... this could be useful for the committee as well for a second look") — see §4 for the full table. PM ruling: visibility only for now, not fed to the R2 committee vote as an input; a second option (surfacing the flag to the voting seats themselves) was considered and deferred. PM then asked for the table to stop being hand-built ("I want it automated, not by hand, which means gaps and forget will occur"): `pma_pipeline.py` now ships a `repeat-watch` subcommand that generates §4 mechanically from `phase4_ledger.json` + `verdict_ledger.json`, and `record-verdicts` now locks EVERY Phase-4 name every run — deliberated verdicts (ADVANCE/HOLD/PASS) and cap-dropped names (NEAR-MISS) alike — so there is no longer a class of ticker that falls through and needs hand reconstruction later. 2026-08-17 was seeded as a one-time historical backfill (declared, tagged `record_source: backfilled_manual_2026-08-18`, distinct from live rows); every session from 2026-08-18 forward locks itself the day it happens. S7Q gate now has a Q6r family that fails the brief if `repeat_watch.json` doesn't cover every repeat-flagged ticker or if a flagged ticker's data doesn't actually appear in the brief text — so a hand-typed §4 that skips this tool can no longer ship clean.
