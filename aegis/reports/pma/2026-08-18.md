...
*Mechanical, ticker-level persistence signal from the Phase-4 qualifying ledger — independent of today's verdict, shown here so a repeat doesn't require digging into disclosures to find. Window is currently only 2 sessions deep (the ledger started 2026-08-17); it will deepen to a true 5-session view as more sessions run — it does not wait for 5 sessions to say something.*

| Ticker | Appearances | Window | Today's verdict |
|---|---|---|---|
| **CVX** | 2/2 | 2 sessions | ADVANCE |
| **COLB** | 2/2 | 2 sessions | ADVANCE |
| **XOM** | 2/2 | 2 sessions | HOLD-FOR-CONDITIONS |
| **IBKR** | 2/2 | 2 sessions | HOLD-FOR-CONDITIONS |

Aug-17 verdicts are not retrievable retroactively — the verdict accountability ledger (§11) only began locking verdicts today. From today forward, every repeat name here will carry its full verdict history alongside the appearance count, not just the count.

## 5 · ACTIONABLE IDEAS — 6 ADVANCE cleared consensus today (soft cap is 5; disclosed, not trimmed — see rationale below)
...
## 6 · HOLD-FOR-CONDITIONS (9 names) — condition line mandatory on each, synthesized from the seat's own R2 falsifier where filed
...
## 7 · PASSED (5 names) — declared, not silently dropped
...
## 8 · NEAR MISS (cut by the cap, not by quality)
...
## 11 · GAPS, RESIDUALS & PROCESS DISCLOSURES — declared, not hidden
...
- **Sector concentration, independently found (Steenbarger SF1).** Energy = 5/20 and Financials = 7/20 of today's deliberation set — 60% of the shortlist in two sectors, one (Financials) carrying macro CAUTION from Druckenmiller. Both concentrations are named against specific tickers in §5/§6 above rather than asserted in the abstract.
