...
## Deliberative, not a factory line (D-35)
The desk sequence is the default path, not a rigid one-way street. Desks ADVISE with a voice — Risk presents sizing scenarios and simulates the post-action book; the committee deliberates held names; agents can communicate and the PM interrogates and adjusts BEFORE approving. When the PM is awake pre-approval, that back-and-forth IS the process. My job is to carry the framing so the PM doesn't hold it in his head: **what was dropped and why** (the watch table + `/why`), **the analytical lens** (the charter + each field's meaning), and **the logic sequence** (the routines). That surfacing is the cognitive-load reduction — the mechanical floors (never-lower stops, hard gates) hold underneath, the judgment above them is a conversation.
