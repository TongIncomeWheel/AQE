...
# ------------------------------------------- COB daily signature (per-ticker)

def test_daily_signature_needs_no_time_normalisation():
    """The completed-bar twin of the live read.

    The sqrt(elapsed) machinery exists only because a LIVE quote measures a
    half-finished day. At COB the session is over, so range/ATR is directly
    meaningful — and a normal day must read ~1.0 with no signature.
    """
    r = I.daily_signature(100, 102.0, 98.5, 100.4, 1.0e6, 1.0e6, 3.5)
    assert r["day_range_atr"] == pytest.approx(1.0, abs=0.02)
    assert r["day_signature"] is None


def test_daily_signature_reads_coil_thrust_and_failed_push():
    coil = I.daily_signature(100, 100.9, 99.6, 100.8, 0.8e6, 1e6, 3.5)
    assert coil["day_signature"] == "COIL"
    thrust = I.daily_signature(100, 106.0, 99.5, 105.7, 1.9e6, 1e6, 3.5)
    assert thrust["day_signature"] == "THRUST"
    fail = I.daily_signature(100, 106.0, 99.5, 100.0, 1.7e6, 1e6, 3.5)
    assert fail["day_signature"] == "FAILED_PUSH"


def test_daily_position_is_the_close_location_value():
    on_highs = I.daily_signature(100, 105, 100, 104.9, None, None, 3.5)
    on_lows = I.daily_signature(100, 105, 100, 100.1, None, None, 3.5)
    assert on_highs["day_position"] > 0.95
    assert on_lows["day_position"] < 0.05


def test_daily_signature_degrades_on_a_missing_or_zero_range():
    assert I.daily_signature(None, None, None, None, None, None, None)["day_signature"] is None
    flat = I.daily_signature(100, 100, 100, 100, 1e6, 1e6, 3.5)
    assert flat["day_position"] is None      # zero range is not "on the lows"


def test_daily_signature_ships_on_every_export_row():
    """A per-ticker DETAIL — present (even as null) rather than sometimes absent."""
    from src.data.drive_sync import _v21_record_fields
    f = _v21_record_fields("TEST", {"entry": 100.0, "atr14": 2.0, "fib": {}},
                           {}, {}, {})
    for k in ("day_signature", "day_position", "day_range_atr", "day_vol_x",
              "day_from_open_pct"):
        assert k in f
