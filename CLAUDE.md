...
- `sector_mapper.py` — GICS sector map, round-trips a rich RAG file via a dedicated Drive subfolder. `universe.py` — fixed, manually-curated ticker universe (CSV synced from a dedicated Drive subfolder); the FMP-screener auto-refresh path exists in code but is **intentionally disabled** (ballooned to ~1800 names) — don't wire it back up without asking.
