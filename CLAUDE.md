...
- `ptj.py` — reads the daily held-positions journal (PTJ) from a dedicated Drive folder (`GDRIVE_PTJ_FOLDER_ID`, default `1AUWXhE…`), picking the **latest-modified** file (dedupes runtime duplicates). Extracts `open_positions` → caches `output/held_positions.json`. The export then sets `held=true` on those tickers and adds a top-level `held_positions` array = the trade (entry/qty/SL/TP/unrealised) + AQE's current engine read (scores, MP state, DSL bracket, sector, RS). Surfaced on the Scanner "Held positions" panel + the Charts "Bought @" overlay.
