...
- **Three letter sections** (from Nick Crown's own macro letter, 2026-08-10): `what_changed` (diff vs the previous run — only moves that change what a PM does; silence is a real answer), `calendar` (CPI/PPI/FOMC/NFP from FMP's economic calendar + earnings filtered to HELD then daily-list names, each with what it *tests*, never a forecast), and **`key_levels`** — one table sorted nearest-first where **most rows are NOT prices**: the RSP/SPY 12-month range + 20MA, the volatility gap's own percentile bands, VIX 15/25, implied-correlation, alongside gamma flip/walls and CTA flip levels. Every row carries `if_it_breaks`.
- **Daily: steps 6f (Crown, gamma ON) / 6g (scenarios) / 6h (reading copy → Drive).** Same `run_daily.bat`, no new command. **`aqe_crown_macro.json`** is the LLM/committee file — published to the pinned Drive folder beside `aqe_daily_export.json`, plain English first (`read_me_first`), chart series stripped, ~14KB. `crown_macro.json` stays local and carries the series for the page. All five artifacts ride in Daily Persist. Full doc: `docs/AQE_CROWN_DAILY_AND_OUTPUT.md`.
- Page: 🫀 Crown Macro. Daily: step 6f (gamma ON, wrapped like QS). Full doc: `docs/AQE_CROWN_MACRO_LAYER.md`.
...
### Macro pack (`src/macro/pack.py`) — the fifth, read-only door
Signed off 2026-08-13 (`docs/AQE_MACRO_PACK_PROPOSAL.md`). Reads Crown, the scenario layer, SRM and Thematic Rotation's already-finished output — adds exactly one new computation, never changes any of the four: whether each sector's/theme's current grade agrees with what the leading scenario implies for it (`sector_read`/`thematic_read`, categorical AGREES/DISAGREES/UNTESTED, never numeric). `crown_status` (and a missing leading scenario) gates the whole artifact exactly as it gates Crown alone — on `EARLY_EXIT`/`UNAVAILABLE`, or no scenario leading, `pack_status: PARTIAL` and `sector_read`/`thematic_read` are **absent**, not empty. `pack_status`/`crown_status`/`oldest_leg` surface at the top level, same discipline as the Committee Card. Runs as **Step 8a-1** in the daily orchestrator, after Step 8 (export write, since SRM/thematic grades live there) → `output/aqe_macro_pack.json`, published + persisted alongside the other four macro artifacts. Full doc: `docs/AQE_CROWN_DAILY_AND_OUTPUT.md` §1.

### VALEN Dashboard (`src/valen/`) — VIV System Part 1, ported
Signed off 2026-09-22 (`docs/AQE_VALEN_DASHBOARD_PROPOSAL.md`). Ports the *VIV System* handbook's Situational Awareness card (market trend, extension, breadth, rotation) into AQE's own idiom — **Part 1 (Weather) only**; parts 2-6 stay where AQE already implements them (longlist/Elder/QS, DETECT+patterns, `bracket_engine`). `spec.py` is the frozen threshold transcription (10/20-day SMA cross, VIX/VIX3M bands, ATR-multiple-from-50-day, the six-row checklist, the four whole-market breadth instruments) — same no-tuning discipline as `qs_spec.py`. `trend.py` (SPY/QQQ, daily+weekly, from the same panel every engine reads), `extension.py` (VIX/VIX3M via `crown.vol`, index ATR-multiple, curated-panel % above 20-day as labelled **context**), `stance.py`, `explain.py` (rule-based, no LLM, same `headline/because/so_what/watch_for/caveats/as_of/note` shape as `crown/explain.py`), `card.py` (renders from the artifact alone — no pandas, no file access, same blindness rule as `qs_card.py`), `live.py` (market-hours refresh, reusing `alerts.engine.in_market_window()` + the previously-unused `FMPClient.get_quotes_batch()`), `daily.py` (orchestration + `output/valen_dashboard.json` local / `aqe_valen_dashboard.json` published split).
- **Two constraints resolved at design time, not discovered in production.** (1) VALEN's stance *prescribes size* (RISK ON = full playbook, NEUTRAL = half size); AQE's charter forbids sizing/disposition (the `max_new_size` removal, 2026-08-13). Resolution: `stance` ships as a market **reading** — same category as the existing `regime` field — never a size; the handbook's sizing guidance renders in the UI as quoted doctrine, outside the JSON. (2) VALEN's breadth instruments are **whole-market** (one flip rule needs 350+ monthly 25%-movers — arithmetically unreachable on AQE's ~819-ticker curated panel). Resolution: every breadth-shaped field carries `population` + `n`; a curated-panel number is never labelled `t2108`. **Phase 1 ships breadth as an honest `UNAVAILABLE`** (reason: needs `ma_scanner.py` widened to a full-tape population, PM decision pending) rather than a universe-scoped number wearing the real instrument's name — so `stance.status` is `DEGRADED` (no one-word stance) until that population is wired in, while trend and extension still compute and render.
- **Live during market hours**: click-to-refresh (never auto-polling), matching the house pattern on the Charts & Trade Entry page. Pulls one FMP batch quote (SPY/QQQ/^VIX) and recomputes trend + the index/VIX legs of extension for that page render only — never overwrites the nightly artifact. VIX3M cannot go live (not on the FMP Starter plan; Cboe's file is EOD-only), so a live VIX/VIX3M ratio moves only the VIX leg and says so (`basis: "vix_live_vix3m_eod"`). Whole-market breadth never refreshes live (not a per-click computation) and always shows its last nightly `as_of`.
- Daily: **Step 6i**, after Crown/scenarios/reading-copy (6f-6h), wrapped exactly like Crown/QS — an addition that must never take the export down. Local-file-only for now; Drive/GitHub publish alongside the other four macro artifacts is a Phase-1.5 follow-up once the page is confirmed useful. Page: 🧭 VALEN Dashboard (`src/ui/pages/7_VALEN_Dashboard.py`).
- **Not yet built** (Phase 2/3, needs a PM call on the breadth substrate first): the four whole-market instruments for real, cross-group Theme Leaders / Rotation ranking, the `in_theme` flag on `daily_list`, and the no-buy-list checklist.

### Engines (`src/engines/`)
Flow / Energy / Structure / MP / Elder / BQ / K39 / Pipeline Rank / SC_MOMENTUM+SC_POSITION composites — full formulas in the reference doc §Part II.1-3. `bracket_engine.py` is **the** stop/target source of truth (structural, 3-charter-gates validated; mechanical DSL/TP fields are retired from the export). `srm.py` — sector grading + RRG + macro overlay + 35 thematic baskets (context layer, never adds scan names). DETECT layer (`divergence.py`, `pin_bar.py`, `smart_money_knn.py`, `signal_radar.py`, `squeeze_breakout.py`, `vwap.py`) and `lens_consensus.py` (the unweighted lens-agreement reading aid) are data-only — never gates, never sizing.
