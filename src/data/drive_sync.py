...
_FIELD_GLOSSARY = {
    ...
    "pattern": "CHART-PATTERN LENS — the classic formation this name is currently "
               "sitting inside, or null. A VISUAL FLAG (PM ruling 2026-08-06), not a "
               "signal: it adds no names, gates nothing, sizes nothing and carries no "
               "probability. Either a shape is there or it is not. CUP_HANDLE / "
    ...
    "pattern_stage": "How far along the shape is. CUP_HANDLE: CUP = both rims formed, no "
                     ...
                     "neckline. ASC_TRIANGLE: FORMING = flat ceiling with rising lows "
                     "under it. TRIGGERED (all three) = price has closed above the "
                     "trigger level.",
    "pattern_trigger": "The level that CONFIRMS the shape (USD): the rim for a cup, the "
                       "NECKLINE (the peak between the two lows) for a double bottom, the "
                       "flat ceiling for an ascending triangle. Same kind of object as "
                       "last_pivot_high.",
    "pattern_invalidation": "Below this the shape is gone whatever the fit score says "
                            "(USD): the handle low (or cup low) for a cup, the lower of "
                            "the two lows for a double bottom, the LAST rising low for an "
                            "ascending triangle — the step that would have to fail.",
    "pattern_days": "Days from the start of the formation to today. A textbook cup runs "
                    "2-6 months, so this is how you tell a real base from a three-week dip.",
    "pattern_fit": "0-1, how TEXTBOOK the shape is — cup: depth, rim symmetry, handle "
                   "size, handle volume; double bottom: how level the two lows are and how "
                   "real the bounce between them; triangle: how flat the ceiling is, how "
                   "far the lows have climbed, how many touches. Flat-weighted. NOT a "
                   "probability and NOT an edge — nothing here says the shape works. It is "
                   "how closely it matches the textbook drawing, nothing more.",
    "pattern_start": "Date the formation began (left rim / first low / first ceiling touch).",
